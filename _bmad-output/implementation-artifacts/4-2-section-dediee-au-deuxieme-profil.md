...
- [ ] **Task 1 — Adapter `components/site-header.tsx` pour les pages sans hero** (AC: #2, #7)
  - [ ] Ajouter une prop (ex. `alwaysVisible?: boolean`, défaut `false`) : quand elle est vraie, le header est rendu visible en permanence, sans `IntersectionObserver` ni dépendance au repère `#hero-logo-sentinel`. Quand elle est fausse, comportement actuel strictement inchangé (y compris le repli « pas de repère → masqué »).
  - [ ] Sur `/camarade-exclu`, le header fixe recouvre le haut de page → compenser (padding-top sur le contenu, ou variante non-`fixed` quand `alwaysVisible`). Choisir la solution la plus simple et lisible (NFR-1).
  - [ ] Optionnel (si trivial) : marquer la page courante dans la nav (`aria-current="page"` sur « Aider un camarade »). Ne pas restructurer la nav au-delà.
- [ ] **Task 2 — Créer `app/camarade-exclu/page.tsx` : structure + repères** (AC: #1, #4, #5, #6)
  - [ ] Server Component **synchrone**, purement présentationnel : aucun accès Supabase, aucune Server Action, aucun `cookies()`, aucun `"use client"` sur la page elle-même.
  - [ ] Exporter `metadata` locale (title + description).
  - [ ] `<main className="flex flex-1 flex-col …">` (pattern du layout `<body>` flex-col, comme `/`), `<SiteHeader alwaysVisible />`, intro (h1 + chapeau), 3–4 repères (h2 + paragraphe — `Card` de la fondation ou prose simple, au choix), section « et si tu ne sais pas quoi faire ? » avec CTA chat, footer identique à celui de `/` (logo + « La Parole Avant Tout © 2026 »).
  - [ ] Intégrer la base éditoriale de Dev Notes § Contenu proposé (adaptable, l'esprit prime). Relire contre l'AC #4 : jamais culpabilisant, aucun numéro d'urgence, rien de « à venir ».
- [ ] **Task 3 — CTA vers le chat (complément, pas substitut)** (AC: #3)
  - [ ] `<Button asChild size="lg"><Link href="/discussion-anonyme">…</Link></Button>` avec un libellé explicite orienté question (ex. « Pose ta question, anonymement ») — cohérent avec « Parler à quelqu'un » de l'accueil mais adapté à ce profil.
  - [ ] Une phrase d'accompagnement : anonyme, sans inscription, une vraie personne du lycée répond — même mécanisme que pour tout le monde.
- [ ] **Task 4 — Vérifier non-régression et build** (AC: #2, #7)
  - [ ] `npm run build` OK, `npm run lint` OK (`.env.local` requis pour builder — condition pré-existante, voir Dev Notes § Build).
  - [ ] Parcours complet : `/` → scroll → header apparaît → clic « Aider un camarade » → page s'affiche (plus de 404) → header visible d'emblée → clic CTA → `/discussion-anonyme` fonctionnel → logo header → retour `/`.
  - [ ] Sur `/` : comportement scroll du header strictement inchangé (masqué en haut, révélé après le logo, re-masqué en remontant).
  - [ ] `/discussion-anonyme`, `/organisateurs/connexion`, `/styleguide` inchangés.
  - [ ] **Checkpoint Charles** : valider le contenu éditorial (les repères), le libellé de nav « Aider un camarade » (provisoire depuis la 4.1) et le libellé du CTA avant de clôturer.
